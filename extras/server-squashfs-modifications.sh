...
apt-get update && \
  apt-get install --no-install-recommends -y bash curl gpg lsb-core software-properties-common proxychains4 && \
  ...
  apt-get update && \
  proxychains4 apt-get install --no-install-recommends -y mokutil efibootmgr network-manager tpm2-tools sox \
